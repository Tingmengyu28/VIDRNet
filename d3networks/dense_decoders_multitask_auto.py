...
class DenseUNet_v2(nn.Module):
    ...
    def __init__(self, d_block, input_nc=3, outputs_nc=[1], growth_rate=32,
        ...
        num_features_fixed = num_features
        # Decoder layer
        for task_i in range(len(tasks)):
            num_features = num_features_fixed
            decoder = nn.Sequential()
            for i in reversed(range(3, 6)): # common decoder until block 3
                mult = 1 if i == 5 else ngf_mult
                dropout = use_dropout if i > 3 else False
                decoder.add_module('d_block{}'.format(i),
                                    self._make_decoder_layer(num_features * mult,
                                    int(num_features / 2), block=d_block,
                                    use_dropout=dropout))
                num_features = num_features // 2
            ...
            decoder.add_module('d_block{}'.format(i-1), # 2
                                  self._make_decoder_layer(num_features * mult,
                                  num_features // 2, block=d_block,
                                  use_dropout=False))
            ...
            decoder.add_module('d_block{}'.format(i-2), # 1
                                self._make_decoder_layer(num_features * mult,
                                                         num_features, block=d_block,
                                                         use_dropout=False))

            decoder.add_module('last_conv',
                                        conv3x3(num_features, outputs_nc[task_i]))

            self.decoder_tasks.append(decoder)
    ...
    def forward(self, x):
        ...
        for task_i in range(len(self.tasks)):
            # input is ngf * 16 x 8 x 8
            out_task = self.decoder_tasks[task_i].d_block5(out)
            # input is (ngf * 8) x 16 x 16
            out_task = self.decoder_tasks[task_i].d_block4(self.get_decoder_input(tb_denseblock3, out_task))
            # input is (ngf * 4) x 32 x 32
            out_task = self.decoder_tasks[task_i].d_block3(self.get_decoder_input(tb_denseblock2, out_task))
            # input is (ngf * 2) x 64 x 64
            out_task = self.decoder_tasks[task_i].d_block2(self.get_decoder_input(tb_denseblock1, out_task))
            # input is ngf x 128 x 128
            out_task = self.decoder_tasks[task_i].d_block1(self.get_decoder_input(out_conv1, out_task))
            # input is ngf x 256 x 256
            out_task = self.decoder_tasks[task_i].last_conv(out_task)
            
            if self.tasks[task_i] == 'reconstruction':
                out_task += x
