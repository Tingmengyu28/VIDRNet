...
class GaussPSF(nn.Module):
    def __init__(self, kernel_size, near=1, far=10, pixel_size=56e-7, scale=3):
    ...
    def forward(self, image, depth, focal_depth, apature, focal_length):
        ...
        beta = torch.abs(Ap * (FL * (real_depth - real_fdepth)) / (real_depth * (real_fdepth - FL))) / (self.pixel_size * self.scale)
        c = beta.clamp(min=1, max=self.kernel_size)
        weights = c.unsqueeze(1).expand_as(image).contiguous()

        return GaussPSFFunction.apply(image, weights, self.kernel_size), beta / 2000
